...
function forceSyncSchema() {
  ...
  try {
    ...
    if (!blCols.includes("price_type")) {
      db.exec(
        "ALTER TABLE base_library ADD COLUMN price_type TEXT DEFAULT 'AREA'"
      );
    }
    if (!blCols.includes("agency_price")) {
      db.exec(
        "ALTER TABLE base_library ADD COLUMN agency_price REAL DEFAULT 0"
      );
    }
    ...
    if (!fdInfo.some((c) => c.name === "is_active")) {
      db.exec(
        "ALTER TABLE fee_definitions ADD COLUMN is_active INTEGER DEFAULT 1"
      );
    }
    ...
    if (categoryCount === 0) {
      ...
      const insertItem = db.prepare(
        "INSERT INTO base_library (name, category_id, price_type, unit_name, agency_price, guide_price) VALUES (?, ?, ?, ?, ?, ?)"
      );
    ...
    if (feeCount === 0) {
      ...
      const insertFee = db.prepare(
        "INSERT INTO fee_definitions (name, default_rate, is_active) VALUES (?, ?, ?)"
      );
...
export function setupIpcHandlers() {
  ...
  ipcMain.handle("db:addCategory", (_, d) =>
    db
  ...
  // 2. 基础库单项 (支持单价与计价类型)
  ipcMain.handle("db:getBaseLibrary", () =>
    db
      .prepare(
        "SELECT bl.*, c.name as categoryName FROM base_library bl JOIN categories c ON bl.category_id = c.id"
      )
      .all()
  );
  ...
  ipcMain.handle("db:deleteBaseItem", (_, id) =>
    db.prepare("DELETE FROM base_library WHERE id = ?").run(id)
  );
  ipcMain.handle("db:updateBaseItem", (_, d) => {
    if (d.table === "fee_definitions")
      return db
        .prepare(
          "UPDATE fee_definitions SET name = ?, default_rate = ? WHERE id = ?"
        )
    ...
    return db
      .prepare(
        "UPDATE base_library SET name = ?, category_id = ?, price_type = ?, unit_name = ?, agency_price = ?, guide_price = ? WHERE id = ?"
      )
      .run(
        d.name,
        d.category_id,
  ...
  ipcMain.handle("db:addPricedAsset", (_, { name, color, componentIds }) => {
    const transaction = db.transaction(() => {
      const info = db
        .prepare(
          "INSERT INTO priced_assets (display_name, color) VALUES (?, ?)"
        )
        .run(name, color);
      const assetId = info.lastInsertRowid;
      const insertMap = db.prepare(
        "INSERT INTO asset_component_map (asset_id, base_item_id) VALUES (?, ?)"
      );
  ...
  ipcMain.handle("db:getAssetComponents", (_, assetId) =>
    db
      .prepare(
        `
  ...
  ipcMain.handle("db:deletePricedAsset", (_, id) =>
    db.prepare("DELETE FROM priced_assets WHERE id = ?").run(id)
  );
  ipcMain.handle("db:updatePrice", (_, { comboId, field, value }) => {
    const colMap = { color: "color", displayName: "display_name" };
    if (!colMap[field]) return { changes: 0 };
    return db
  ...
  // 4. 费率管理
  ipcMain.handle("db:getFeeDefinitions", () =>
    db.prepare("SELECT * FROM fee_definitions").all()
  );
  ipcMain.handle("db:addFee", (_, { name, rate }) => {
    return db
      .prepare(
        "INSERT INTO fee_definitions (name, default_rate, is_active) VALUES (?, ?, 1)"
      )
      .run(name, rate);
  });
  ...
  ipcMain.handle("db:saveTemplate", (_, { name, bom, width, height }) => {
    ...
    return db
      .prepare(
        "INSERT INTO templates (id, name, bom_json, width, height) VALUES (?, ?, ?, ?, ?)"
      )
      .run(id, name, JSON.stringify(bom), width, height);
  });
  ipcMain.handle("db:deleteTemplate", (_, id) =>
  ...
  ipcMain.handle("db:exportDatabase", async () => {
    const { filePath } = await dialog.showSaveDialog({
      title: "导出备份",
      defaultPath: `moser_glass_backup_${
