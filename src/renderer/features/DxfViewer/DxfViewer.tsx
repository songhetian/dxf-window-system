...
export const DxfViewer = forwardRef<DxfViewerRef, DxfViewerProps>(({ processedResult, windows, focusedMarker = null }, ref) => {
  ...
  useImperativeHandle(ref, () => ({ 
    ...
    zoomToWindow: (win: WindowItem) => {
      ...
      const rect = containerRef.current.getBoundingClientRect();
      const pts = win.points;
      const cx = pts.reduce((a: number, b) => a + b.x, 0) / pts.length;
      const cy = pts.reduce((a: number, b) => a + b.y, 0) / pts.length;
