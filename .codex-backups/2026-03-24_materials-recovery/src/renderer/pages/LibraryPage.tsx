...
const LibraryPage = () => {
  ...
  const addProductItem = () => {
    ...
    setProductItems((items) => [...items, { materialId: selectedMaterialId, quantity: selectedQuantity }]);
    ...
    try {
      ...
      for (const [index, name] of defaultCategories.entries()) {
        if (!categories.some((item) => item.name === name)) {
          await createCategory.mutateAsync({ name, sortOrder: index });
        }
      }
                ...
                <Stack>
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      await createCategory.mutateAsync({ name: categoryName.trim(), sortOrder: categories.length });
                      setCategoryName('');
                    }}
                  >
                    保存分类
