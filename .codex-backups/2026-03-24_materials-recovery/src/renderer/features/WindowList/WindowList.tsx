...
import { useWindowStore, formatUnit, formatArea, getUnitSymbol, getAreaSymbol } from '../../stores/windowStore';
    ...
    <Paper withBorder radius="sm" shadow="none" style={{ 
      ...
      <Box p="sm" style={{ 
        ...
        <Stack gap="xs">
          ...
          <Grid gutter="xs">
            <Grid.Col span={4}><DataTag label="宽度" value={formatUnit(group.width, unit)} unit={unitSym} color="blue" /></Grid.Col>
            <Grid.Col span={4}><DataTag label="高度" value={formatUnit(group.height, unit)} unit={unitSym} color="blue" /></Grid.Col>
            <Grid.Col span={4}><DataTag label="数量" value={group.items.length} unit="樘" color="blue" /></Grid.Col>
          </Grid>
        </Stack>
      </Box>
...
const WindowItemCard = memo(({ win, idx, unit, unitSym, areaSym, activeWindowId, setActiveWindowId, onFocus, onEdit }: any) => (
  ...
  >
    <Stack gap={6}>
      ...
      <Grid gutter={4}>
        <Grid.Col span={6}><DataTag label="宽" value={formatUnit(win.width, unit)} unit={unitSym} /></Grid.Col>
        <Grid.Col span={6}><DataTag label="高" value={formatUnit(win.height, unit)} unit={unitSym} /></Grid.Col>
        <Grid.Col span={6}><DataTag label="面积" value={formatArea(win.area, unit)} unit={areaSym} /></Grid.Col>
        <Grid.Col span={6}><DataTag label="类型" value={win.category} unit="" /></Grid.Col>
      </Grid>
    </Stack>
  </Paper>
));
