...
export const useDxfProcessor = () => {
  ...
  const [pendingWindows, setPendingWindows] = useState<WindowItem[]>([]);
  ...
  const runRecognition = useCallback(async (
    ...
    try {
      ...
      for (let i = 0; i < transformedEntities.length; i += 1) {
        ...
        if (i % BATCH_SIZE === 0) {
          setProgress(20 + Math.floor((i / transformedEntities.length) * 30));
  ...
  }, [identRules, profileWidth, scaleFactor, unitWeight]);
